...
public class ExperimentalTest {
    ...
    public static void main(String[] args) {
        // --> First Dive
        FirstDive dive = new FirstDive();
        dive.setDepth();
        dive.setTime();
        // --> End of First Dive
        System.out.println();
        // --> Start of Second Dive
        MultipleDives dive2 = new MultipleDives();
        dive2.setDepth();
        dive2.setTime();
        dive2.getSurfaceInterval();
            // Add Pressure Group after Dive pls
        
        System.out.println("===== Dive Summary =====");
        System.out.println("\nFirst Dive : ");
        System.out.println("Depth of " + dive.getDepth());
        System.out.println("Time  of " + dive.getTime());
        System.out.println("Ending Pressure Group after Dive " + dive.getPressureGroup());
        
        dive2.getSurfaceInterval();
        
        System.out.println("\nSecond Dive : ");
        System.out.println("Starting Pressure Group after Surface Interval " + dive2.getPressureGroup());
        System.out.println("Depth of " + dive2.getDepth());
        System.out.println("Time  of " + dive2.getTime());
        System.out.println("Ending Pressure Group after Dive " + dive2.getNewPressureGroup());
        
        // 
        
    }
    
    public static class FirstDive {
        static int depth,time,maxNDL;
        static void setDepth() {
            depth  = inputCheck("Please input depth value between ( 1 - " + 42 + " ) meters : " , 1 , 42);
            depth  = RoundUp(depth,depths);
            maxNDL = bottomTime[depth][bottomTime[depth].length - 1];
            System.out.println("Rounded Depth is " + getDepth());
            System.out.println(getNDL());
        }
        static void setTime() {
            time = inputCheck("Please input actual bottom time value between ( 1 - " + maxNDL + " ) minutes : " , 1 ,  maxNDL );
            time = RoundUp(time,bottomTime[depth]);
            System.out.println("Rounded time is " + getTime());
            System.out.println("Your Pressure Group after this dive is " + getPressureGroup());
        }
        private static String getDepth() {
            return depths[depth] + " meters";
        }
        private static String getNDL(){
            return "Depth of " + getDepth() + " has maximum No Decompression Limit of " + maxNDL + " minutes" + "\n";
        }
        private static String getTime() {
            return bottomTime[depth][time] + " minutes";
        }
        private static String getPressureGroup() {
            return pressureGroup[time];
        }
    }
    
    public static class MultipleDives {
        static int depth,time,maxNDL,rnt,adjustedTime;
        static void setDepth() {
            if (FirstDive.depth == 0) {
                System.out.println("\nYou can't dive anymore");
                System.exit(0);
            }
            depth  = inputCheck("Please input depth value between ( 1 - " + depths[FirstDive.depth - 1] + " ) meters : " , 1 , depths[FirstDive.depth - 1]);
            depth  = RoundUp(depth,depths);
            maxNDL = andl[depth]; // ANDL array
            System.out.println("Rounded Depth is " + getDepth());
            System.out.println(getNDL());
        }
        static void setTime() {
            time = inputCheck("Please input actual bottom time value between ( 1 - " + maxNDL + " ) minutes : " , 1 ,  maxNDL );
            time = RoundUp(time,abt[depth]); // ABT array
            System.out.println("Rounded time is " + getTime() + " minutes");
            System.out.println("Your Pressure Group after this dive is " + getPressureGroup());
            setRNT();
            getNewPressureGroup();
        }
        static void getSurfaceInterval() {
            System.out.println("\nSurface Interval between dives : " + setSurfaceInterval());
        }
        public static int setRNT() {
            return rnt = bottomTime[depth][bottomTime[depth].length - 1] - getTime();
        }
        private static String getNewPressureGroup() {
            adjustedTime = RoundUp( (getTime() + setRNT()) ,bottomTime[depth]);
            return pressureGroup[adjustedTime];
        }
        private static String getDepth() {
            return depths[depth] + " meters";
        }
        ...
        private static int getTime() {
            return abt[depth][time];
        }
        private static String getPressureGroup() {
            return pressureGroup[ (abt[depth].length-1) - time ];
        }
        public static String setSurfaceInterval() {
            return surfaceTime[FirstDive.time][(abt[depth].length-1) - time];
        }
    }
