...
public class ExperimentalTestClean {
    ...
    public static void main(String[] args){
        /*
        Program Calculates Shortest Surface Time Interval between Dives.
        Use at Caution    
        */
        // SAC has not been added
        // Tank has not been implemented
        // First Dive only needs Depth + Time.
        DivingCalculator.FirstDive dive = new DivingCalculator.FirstDive();
        dive.setDepth();
        dive.setTime();
    ...
    public static class DivingCalculator{
        ...
        static int depth,time,maxNDL;
            static void setDepth() {
               ...
               System.out.println("Rounded Depth is " + depths[depth] + " meters");
               System.out.println("Depth of " + depths[depth] + " meters has maximum No Decompression Limit of " + maxNDL + " minutes" + "\n");
            }
            static void setTime() {
               ...
               System.out.println("Rounded time is " + bottomTime[depth][time] + " minutes");
               System.out.println("Your Pressure Group after this dive is " + pressureGroup[time]);
            }
        }   
    
        public static class MultipleDives {
            static int depth,time,maxNDL,rnt,adjustedTime;
            static void setDepth(int a) { // Too limiting user. If last dive depth = 10, cant dive anymore even though user has not exceeded the limit of diving each day.
                if (a - 1 < 0) {
                    System.out.println("\nYou can't dive anymore");
                    System.exit(0);
                }
                depth  = inputCheck("Please input depth value between ( 1 - " + depths[a - 1] + " ) meters : " , 1 , depths[a - 1]);
                depth  = RoundUp(depth,depths);
                maxNDL = andl[depth]; // ANDL array
                System.out.println("Rounded Depth is " + depths[depth] + " meters");
                System.out.println("Depth of " + depths[depth] + " meters has maximum Adjusted No Decompression Limit of " + maxNDL + " minutes" + "\n");
            }
            static void setTime() {
                time = inputCheck("Please input actual bottom time value between ( 1 - " + maxNDL + " ) minutes : " , 1 ,  maxNDL );
                time = RoundUp(time,abt[depth]); // ABT array
                rnt = bottomTime[depth][bottomTime[depth].length - 1] - abt[depth][time]; // Saves in minutes
                adjustedTime = RoundUp( (abt[depth][time] + rnt) , bottomTime[depth]); // Find (Rounded Up Time + RNT) to get new time and find it in the array.
                System.out.println("Rounded time is " + abt[depth][time] + " minutes");
                System.out.println("Your Pressure Group REQUIRED for this dive is " + pressureGroup[(abt[depth].length-1) - time]);
            }
            static void getSurfaceInterval(int a) {
                System.out.println("\nSurface Interval between dives : " + surfaceTime[a][(abt[depth].length-1) - time]);
            }
            static void getNewPressureGroup() {
                System.out.println("Pressure Group after Diving : " + pressureGroup[adjustedTime]);
            }
        }
        ...
        public static int[][] abt = 
            ...
            {4,7}}; // 40
        // Change Later
        public static String[][] surfaceTime = // Has not been changed to integers cause i'm that lazy
                {{"0:00 to 3:00"}, // A
