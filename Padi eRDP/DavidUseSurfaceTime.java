...
public class UseSurfaceTime {
    public static void main(String[] args){
        ...
        String[][] surfaceTimeArray=
            ...
            {"2:35 to 5:35", "1:47 to 2:34", "1:25 to 1:46", "1:16 to 1:24", "1:08 to 1:15", "1:00 to 1:07", "0:53 to 0:59", "0:47 to 0:52", "0:41 to 0:46", "0:35 to 0:40", "0:30 to 0:39", "0:25 to 0:29", "0:20 to 0:24", "0:16 to 0:19", "0:12 to 0:15", "0:08 to 0:11", "0:04 to 0:07", "0:00 to 0:03"}, // R
        ...
        for (int coun = 0; coun<dives-1; coun++){
            //Determine new pressure group with old pressure group's surface time array's surface time
            int locationInSurface = 0;
                for (int count = 0; count<alpha.length; count++){
                    if (pressureGroup == alpha[count]){
                        locationInSurface = count;
                    }
                }
            System.out.println("Please enter total surface time in minutes: ");
            int surfaceTime = input.nextInt();
            String newPressureGroup = "0";
            for (int count = 0; count<surfaceTimeArray[locationInSurface].length;count++){
                int hour1 = Integer.parseInt(slice_range(surfaceTimeArray[locationInSurface][count],0,1));
                int minute1 = Integer.parseInt(slice_range(surfaceTimeArray[locationInSurface][count],2,4));
                int totalMinute1 = hour1*60 + minute1;

                int hour2 = Integer.parseInt(slice_range(surfaceTimeArray[locationInSurface][count],8,9));
                int minute2 = Integer.parseInt(slice_range(surfaceTimeArray[locationInSurface][count],10,12));
                int totalMinute2 = hour2*60 + minute2;

                if(surfaceTime>=totalMinute1 && surfaceTime<=totalMinute2){
                    newPressureGroup = alpha[count];
                    break;
                }
            ...
            //END
            //Determine new NDL using new pressure group and ask for depth and bottom time
            int newNDL = 0;
            ...
            for (int count = 0; count<depths.length; count++){
                if (depth == depths[count]){
                    for (int count2 = 0; count2<alpha.length;count2++){
                        if (newPressureGroup == alpha[count2]){
                            actualBottomTime = array[count][count2];
                            newNDL = array[count][array[count].length - 1] - actualBottomTime;
                        }
                    }
                }
            }
            ...
            System.out.println("Your total bottom time is "+ bottomTime + " minutes");
